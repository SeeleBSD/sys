...
impl<T> Arc<T> {
    ...
    pub fn try_new(contents: T) -> Result<Self, AllocError> {
        ...
        let value = ArcInner {
            ...
            refcount: Opaque::new(unsafe { 1i32 }),
            #[cfg(CONFIG_RUST_EXTRA_LOCKDEP)]
...
impl<T: ?Sized> Clone for Arc<T> {
    fn clone(&self) -> Self {
        ...
        unsafe { bindings::BINDING_refcount_inc_not_zero(self.ptr.as_ref().refcount.get()) };
...
impl<T: ?Sized> Drop for Arc<T> {
    fn drop(&mut self) {
        ...
        let is_zero = unsafe { bindings::BINDING_refcount_dec_and_test(refcount) };
