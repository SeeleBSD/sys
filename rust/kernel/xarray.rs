...
impl<'a, T: ForeignOwnable> Drop for Guard<'a, T> {
    fn drop(&mut self) {
        ...
        unsafe { bindings::BINDINGS_xa_unlock(self.1.xa.get()) };
    }
}
...
impl<'a, T: ForeignOwnable> Reservation<'a, T> {
    ...
    pub fn store(self, value: T) -> Result<usize> {
        if self.0.replace(self.1, value)?.is_some() {
            crate::err!("XArray: Reservation stored but the entry already had data!\n");
...
impl<'a, T: ForeignOwnable> Drop for Reservation<'a, T> {
    fn drop(&mut self) {
        if self.0.remove(self.1).is_some() {
            crate::err!("XArray: Reservation dropped but the entry was not empty!\n");
        }
    }
}
...
impl<T: ForeignOwnable> XArray<T> {
    ...
    pub fn replace(self: Pin<&Self>, index: usize, value: T) -> Result<Option<T>> {
        ...
        let ret = unsafe { bindings::BINDINGS_xa_err(old) };
        if ret != 0 {
    ...
    pub fn get(self: Pin<&Self>, index: usize) -> Option<Guard<'_, T>> {
        ...
        unsafe { bindings::BINDINGS_xa_lock(self.xa.get()) };

        // SAFETY: `self.xa` is always valid by the type invariant.
        let guard = ScopeGuard::new(|| unsafe { bindings::BINDINGS_xa_unlock(self.xa.get()) });

        // SAFETY: `self.xa` is always valid by the type invariant.
        let p = unsafe { bindings::BINDINGS_xa_load(self.xa.get(), index.try_into().ok()?) };
    ...
    pub fn remove(self: Pin<&Self>, index: usize) -> Option<T> {
        let p = unsafe { bindings::BINDINGS_xa_erase(self.xa.get(), index.try_into().ok()?) };
        if p.is_null() {
            None
        } else {
...
impl<T: ForeignOwnable> Drop for XArray<T> {
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            bindings::BINDINGS_xa_empty(self.xa.get());
