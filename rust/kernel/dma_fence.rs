...
pub trait RawDmaFence: crate::private::Sealed {
    ...
    fn set_error(&self, err: Error) {
        unsafe { bindings::BINDING_dma_fence_set_error(self.raw(), err.to_errno()) };
    }
}
...
unsafe extern "C" fn release_cb<T: FenceOps>(fence: *mut bindings::dma_fence) {
    ...
    unsafe { bindings::BINDING_dma_fence_free(fence) };
}
...
impl FenceChain {
    ...
    pub fn new() -> Result<Self> {
        ...
        let ptr = unsafe { bindings::BINDING_dma_fence_chain_alloc() };
...
impl Drop for FenceChain {
    fn drop(&mut self) {
        ...
        unsafe { bindings::BINDING_dma_fence_chain_free(self.ptr) };
    }
}
