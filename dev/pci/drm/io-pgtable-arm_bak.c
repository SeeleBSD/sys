...
/* IOPTE accessors */
#define iopte_deref(pte,d) __arm_lpae_iopte_deref(pte, d)
...
struct arm_lpae_io_pgtable {
	...
	struct arm_lpae_dma_pgtable    *pgd;

	struct arm_lpae_io_pgtable *child;
};
...
__arm_lpae_iopte_deref(arm_lpae_iopte pte, struct arm_lpae_io_pgtable *data)
{
	...
	return (arm_lpae_iopte *)((vaddr_t)data->pgd->cpu_addr + ((voff_t)paddr - (voff_t)__arm_lpae_dma_addr(data->pgd->cpu_addr)));
}

static inline arm_lpae_iopte
cmpxchg64_relaxed(arm_lpae_iopte *ptr, arm_lpae_iopte old, arm_lpae_iopte new)
{
	return atomic_cas_ulong((unsigned long *)ptr, (unsigned long)old, (unsigned long)new);
}
...
#else
static struct arm_lpae_dma_pgtable *
__arm_lpae_alloc_pages(size_t size, int flags,
                       struct io_pgtable_cfg *cfg,
                       void *cookie)
{
...
out_destroy_dmamap:
    bus_dmamap_destroy(dmat, pgtable->dmamap);
out_unmap:
    bus_dmamem_unmap(dmat, pgtable->cpu_addr, size);
out_free_dmamem:
    bus_dmamem_free(dmat, pgtable->segs, pgtable->nsegs);
out_free_pgtable:
    free(pgtable, M_DEVBUF, sizeof(*pgtable));
    return NULL;
}
#endif
...
#else
static void
__arm_lpae_free_pages(struct arm_lpae_dma_pgtable *dma, size_t size,
			struct io_pgtable_cfg *cfg,
			void *cookie)
{
	bus_dma_tag_t dmat = dma->dmat;
	bus_dmamap_t dmamap = dma->dmamap;

	bus_dmamap_unload(dmat, dmamap);
	bus_dmamap_destroy(dmat, dmamap);
	bus_dmamem_unmap(dmat, dma->cpu_addr, size);
	bus_dmamem_free(dmat, dma->segs, dma->nsegs);
...
#else
static void __arm_lpae_clear_pte(arm_lpae_iopte *ptep, struct io_pgtable_cfg *cfg,
                                struct arm_lpae_dma_pgtable *dma)
{

	*ptep = 0;

	if (!cfg->coherent_walk)
		__arm_lpae_sync_pte(ptep, 1, dma);
}
#endif
...
#else
		__arm_lpae_sync_pte(ptep, num_entries, data->pgd);
#endif
}
...
static arm_lpae_iopte arm_lpae_install_table(struct arm_lpae_dma_pgtable *table,
					     arm_lpae_iopte *ptep,
					     arm_lpae_iopte curr,
					     struct arm_lpae_io_pgtable *data)
{
	...
	new = paddr_to_iopte(__arm_lpae_dma_addr(table->cpu_addr), data) | ARM_LPAE_PTE_TYPE_TABLE;
			  ...
			  gfp_t gfp, size_t *mapped)
{
...
#else
    struct arm_lpae_dma_pgtable *cpg;
		arm_lpae_iopte *cptep, pte;
#endif
	...
	/*int range_sat = 1;
	if (data->child) {
		range_sat = ((paddr_t)__arm_lpae_dma_addr(data->child->pgd->cpu_addr) <= iopte_to_paddr(pte, data->child)
	                 && iopte_to_paddr(pte, data->child) < (paddr_t)__arm_lpae_dma_addr(data->child->pgd->cpu_addr) + (paddr_t)data->child->pgd->size);
	}*/
	if (!pte || (pte && !data->child) /* || (pte && !range_sat)*/) {
		cpg = __arm_lpae_alloc_pages(tblsz, gfp, cfg, data->iop.cookie);
		if (!cpg)
			return -ENOMEM;
		cptep = cpg->cpu_addr;
		
		pte = arm_lpae_install_table(cpg, ptep, 0, data);
		if (pte)
			__arm_lpae_free_pages(cpg, tblsz, cfg, data->iop.cookie);
	...
	if (pte && !iopte_leaf(pte, lvl, data->iop.fmt)) {
		if (data->child /*&& range_sat*/) {
#ifdef __linux__
		cptep = iopte_deref(pte, data);
#else
			cpg = data->child->pgd;
			cptep = iopte_deref(pte, data->child);
			printf("cptep: %p pte paddr: 0x%lu\n", cptep, iopte_to_paddr(pte, data->child));
		}
#endif
	} else if (pte) {
		/* We require an unmap first */
	...
	if (!data->child) {
		struct arm_lpae_io_pgtable *new_data;
		new_data = malloc(sizeof(*new_data), M_DEVBUF, M_NOWAIT | M_ZERO);
		if (new_data == NULL)
			return -ENOMEM;
	...
	/* Rinse, repeat */
	return __arm_lpae_map(data->child, iova, paddr, size, pgcount, prot, lvl + 1,
			      cptep, gfp, mapped);
}
...
#else
  arm_lpae_iopte *ptep = data->pgd->cpu_addr;
#endif
				    ...
				    arm_lpae_iopte *ptep)
{
	...
	while (ptep != end) {
		...
		if (data->child)
			__arm_lpae_free_pgtable(data->child, lvl + 1, iopte_deref(pte, data->child));
		else
...
#else
	__arm_lpae_free_pages(data->pgd, table_size, &data->iop.cfg, data->iop.cookie);
#endif
}
...
static void arm_lpae_free_pgtable(struct io_pgtable *iop)
{
	...
	__arm_lpae_free_pgtable(data, data->start_level, data->pgd->cpu_addr);
	kfree(data);
}
				       ...
				       arm_lpae_iopte *ptep, size_t pgcount)
{
...
#else
    struct arm_lpae_dma_pgtable *tablep;
    arm_lpae_iopte *nptep;
#endif
...
#else
		__arm_lpae_init_pte(data, blk_paddr, pte, lvl, 1, &tablep->cpu_addr[i]);
#endif
	}
	...
	return __arm_lpae_unmap(data, gather, iova, size, pgcount, lvl, nptep);
}
			       ...
			       int lvl, arm_lpae_iopte *ptep)
{
...
#else
			__arm_lpae_clear_pte(ptep, &iop->cfg, data->pgd);
#endif
	...
	/* Keep on walkin' */
	if (data->child) {
		ptep = iopte_deref(pte, data->child);
		return __arm_lpae_unmap(data->child, gather, iova, size, pgcount, lvl + 1, ptep);
	} else {
...
#else
	arm_lpae_iopte pte, *ptep = data->pgd->cpu_addr;
#endif
				  ...
				  arm_lpae_iopte *ptep, int lvl)
{
	...
	if (data->child) {
		ptep = iopte_deref(pte, data->child);
		return __arm_lpae_iopte_walk_dirty(data->child, walk_data, ptep, lvl + 1);
	} else {
		ptep = iopte_deref(pte, data);
...
#else
	cfg->apple_uat_cfg.ttbr = __arm_lpae_dma_addr(data->pgd->cpu_addr);
#endif
