...
int
BINDING_signal_pending(struct proc* p)
{
	return SIGPENDING(p);
}

bool
BINDING_refcount_dec_and_test(int *p)
{
	return refcount_dec_and_test(p);
}

bool
BINDING_refcount_inc_not_zero(int *p)
{
	return refcount_inc_not_zero(p);
}

void
BINDING_init_waitqueue_head(wait_queue_head_t *wqh)
{
...
void
BINDING_wake_up(wait_queue_head_t *wqh)
{
	wake_up(wqh);
	return;
}
...
void
BINDING_init_wait_entry(wait_queue_entry_t *wqe, int flags)
{
